...
var wrongWords = [];
for (let i = 0; i < 10; i++) {
	let word = wordsArray[randomInt(0, wordsArray.length)];
	if (randomWords.includes(word)) {
		word = wordsArray[randomInt(0, wordsArray.length)];
	}
...
var inputText = document.getElementById("write");
...
function startTimer() {
	if (timer == null) {
		document.getElementsByClassName("wrongs")[0].style.display = "none";
		var curSec = 0;
		var seconds = 60;
		timer = setInterval(() => {
			if (seconds == 0) {
				...
				seconds -= 1;
				document.getElementsByClassName("time")[0].innerHTML = "0:" + seconds;
			}
		}, 1000);
	}
}
